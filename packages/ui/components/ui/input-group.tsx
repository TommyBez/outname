...
const inputGroupButtonVariants = cva(
  'flex items-center gap-2 border-transparent text-sm',
  {
    variants: {
      size: {
        xs: "h-8 gap-1 px-2 has-[>svg]:px-2 [&>svg:not([class*='size-'])]:size-3.5",
        sm: 'h-9 gap-1.5 px-2.5 has-[>svg]:px-2.5',
        'icon-xs': 'size-8 p-0 has-[>svg]:p-0',
...
function InputGroupTextarea({
  ...
  return (
    <Textarea
      className={cn(
        'flex-1 resize-none border-0 bg-transparent py-3 focus-visible:ring-0',
        className
      )}
      data-slot="input-group-control"
      {...props}
