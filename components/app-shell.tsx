...
export function AppShell({
  ...
  return (
    <SidebarProvider defaultOpen>
      <a
        className="sr-only focus:not-sr-only focus:fixed focus:top-4 focus:left-4 focus:z-50 focus:rounded-md focus:bg-foreground focus:px-3 focus:py-2 focus:text-background focus:text-sm focus:shadow-md"
        href="#main-content"
      >
        Skip to content
      </a>
      ...
      <SidebarInset className="min-w-0">
        ...
        <header className="sticky top-0 z-20 flex h-14 items-center gap-2 bg-background/80 px-3 backdrop-blur supports-backdrop-filter:bg-background/60 md:h-12 md:px-4">
          <SidebarTrigger className="-ml-1 size-9 md:size-8" />
          <Link
            className="inline-flex items-center gap-2 font-medium font-mono text-sm uppercase tracking-[0.18em] transition-colors hover:text-foreground/80 md:hidden"
            href="/"
          >
