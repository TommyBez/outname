...
- **Database**: Remote Neon Postgres via `DATABASE_URL` (no local DB required). Server routes use `@/shared/db/pool` (`pg` + `attachDatabasePool` per [Neon + Vercel connection methods](https://neon.com/docs/guides/vercel-connection-methods)). Workflow code uses `@/shared/db` (Neon HTTP driver) because workflow bundles cannot import Node-only `pg`. Use Neon's pooled connection string (hostname includes `-pooler`).
