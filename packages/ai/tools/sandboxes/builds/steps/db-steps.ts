...
import type { dbSchema } from '@outname/db/schema-registry'
import { sendAppRevalidation } from '@outname/shared/server/app-revalidation-client'
import { agentToolsTag } from '@outname/shared/server/cache-tags'
import {
...
async function getDb(): Promise<NodePgDatabase<typeof dbSchema>> {
  const { db } = await import('@outname/db')
  return db
}
...
export async function markBuildReady(input: {
  ...
  await revalidateAgentToolTags(agentIds.map((row) => row.agentId))
}
...
export async function markBuildFailed(input: {
  ...
  if (row) {
    ...
    await revalidateAgentToolTags(agentIds.map((item) => item.agentId))
  }
}

async function revalidateAgentToolTags(agentIds: string[]): Promise<void> {
  const tags = agentIds.map(agentToolsTag)
  for (const tag of tags) {
    revalidateTag(tag, 'max')
  }
  try {
    await sendAppRevalidation({
      tags: tags.map((tag): [string, 'max'] => [tag, 'max']),
    })
  } catch (error) {
    console.error('[tool-sandbox-build] app revalidation failed', error)
  }
}
...
export async function readManifestSetupScript(input: {
  ...
  try {
    const { getToolSandboxSetupScript } = await import(
      '@outname/ai/tools/sandboxes/registry'
    )
