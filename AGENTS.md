...
- **Database**: Remote Neon Postgres via `DATABASE_URL` (no local DB required). Use `@/shared/db` (`pg` + `attachDatabasePool` per [Neon + Vercel connection methods](https://neon.com/docs/guides/vercel-connection-methods)). Use Neon's pooled connection string (hostname includes `-pooler`). Workflow *flow* code may statically import small step modules, but every module reachable from the flow bundle must stay top-level workflow-safe. Do not statically import `@/shared/db` or server modules that do. Keep DB/sandbox work in small `'use step'` helpers under `agent-runtime/workflows/session/steps/db/`, and move any Node-only/server imports behind the step body (for example via `getDb()` or `await import(...)` inside the step).
