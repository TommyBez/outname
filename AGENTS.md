...
- **Database**: Remote Neon Postgres via `DATABASE_URL` (no local DB required). The app uses a `pg` connection pool with Vercel's `attachDatabasePool` for Fluid compute (see [Neon + Vercel connection methods](https://neon.com/docs/guides/vercel-connection-methods)). Use Neon's pooled connection string (hostname includes `-pooler`).
