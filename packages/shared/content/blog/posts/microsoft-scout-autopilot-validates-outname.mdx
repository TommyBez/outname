...
This is not hypothetical. Four days ago, the US government killed Anthropic's Fable 5 model with an export control directive — 72 hours after launch. Every agent that depended on a single model provider went dark. Every agent built on model-agnostic architecture kept running.
